$(document).ready(function () {
   /* var connection = new signalR.HubConnectionBuilder().withUrl("/chatHub").build();*/
    var connection = new signalR.HubConnectionBuilder().withUrl("/GlobalCalendar/chatHub").build();
    ...
    function AppViewModel() {
        ...
        self.sendToRoom = function (room, message) {
            if (room.name()?.length > 0 && message.length > 0) {
                fetch('/GlobalCalendar/api/Messages', {
                    method: 'POST',
        ...
        //    debugger;
        //    fetch('/GlobalCalendar/api/Rooms')
        //        .then(response => response.json())
        //        .then(data => {
        //            self.chatRooms.removeAll();
        //            for (var i = 0; i < data.length; i++) {
        //                console.log(data[i]);
        //                self.chatRooms.push(new ChatRoom(data[i].id, data[i].name, data[i].admin, data[i].cnt));
        //            }
        ...
        //        });
        //}
        // Define a function to fetch room data from the server
        function fetchRoomData() {
            return fetch('/GlobalCalendar/api/Rooms')
                .then(response => {
                    if (!response.ok) {
        ...
        // Define a function to update the chat rooms list
        ...
        // Define a function to periodically update chat rooms
        ...
        // Call updateRoomList initially and then every 2 seconds
        updateRoomList();
        ...
        self.editRoom = function () {
            ...
            fetch('/GlobalCalendar/api/Rooms/' + roomId, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
        ...
        self.deleteRoom = function () {
            fetch('/GlobalCalendar/api/Rooms/' + self.joinedRoom().id(), {
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' },
        ...
        self.deleteMessage = function () {
            ...
            fetch('/GlobalCalendar/api/Messages/' + messageId, {
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' },
        ...
        self.messageHistory = function () {
            fetch('/GlobalCalendar/api/Messages/Room/' + viewModel.joinedRoom().name())
                .then(response => response.json())
                .then(data => {
    ...
    function ChatRoom(id, name, admin,cnt) {
        var self = this;
        self.id = ko.observable(id);
        self.name = ko.observable(name);
        self.admin = ko.observable(admin);
        self.cnt = ko.observable(cnt);
    }
